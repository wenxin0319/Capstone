"""
usage:
    CUDA_VISIBLE_DEVICES=6 python zyk_tgb.py python gnn_tgb_new.py --dataset tgbn-trade

"""
import sys
sys.path.append("/home/zyksir/TGB")
import timeit
import argparse
import logging
import torch
...
"""
@params: 
    data: which has N items
    start_year: [0, start_year] is train data
    end_year: (start_year, end_year] is test data
@return:
    train_data, valid_data
"""
def create_train_val_split(data: TemporalData, train_year: int, test_year: int):
    logging.info(f"[create_train_test_split] create mask for {data} with train_year {train_year} and test_year{test_year}")
    train_data_mask = data.t <= train_year
    test_data_mask = (train_year < data.t) & (data.t <= test_year)
    return data[train_data_mask], data[test_data_mask]


train_data, val_data = create_train_val_split(data, 2011, 2016)
...
def test(loader):
    ...
    for batch in loader:
        ...
        query_t = batch.t[-1]
        logging.info(f"query_t = {query_t}, label_t = {label_t}")
        if query_t > label_t:
            label_tuple = dataset.get_node_label(query_t)
    ...
    metric_dict = {}
    if num_label_ts < 1:
        logging.warning("no label is evaluated")
        return {}
    metric_dict[eval_metric] = total_score / num_label_ts
    return metric_dict
...
def create_test_data(data: TemporalData, train_year: int, test_year: int, p: float) -> TemporalData:
    assert 0 <= p <= 1.0, "ration must be in range of [0, 1]"
    train_data_mask = data.t <= train_year
    test_data_mask = (train_year < data.t) & (data.t <= test_year)
    test_cnt = int((1 - p) * sum(test_data_mask))
    last_year_idx = test_data_mask.nonzero().squeeze()
    last_year_idx = last_year_idx[torch.randperm(last_year_idx.size(0))[:test_cnt]]
    test_data_mask[last_year_idx] = False
    return data[train_data_mask | test_data_mask]


for test_year in [2011, 2012, 2013, 2014]:
    logging.info(f"try to evaluate Time Range [0, {test_year}]:")
    test_data = create_test_data(data, 2011, test_year, 1.0)
    test_loader = TemporalDataLoader(test_data, batch_size=batch_size)
    start_time = timeit.default_timer()
    test_dict = test(test_loader)
    print(test_dict)
    test_curve.append(test_dict[eval_metric])
    logging.info("Test takes--- %s seconds ---" % (timeit.default_timer() - start_time))
    dataset.reset_label_time()

for p in [0.1, 0.5, 0.8]:
    logging.info(f"try to evaluate Time Range [0, 2011] + {p}*(2011, {test_year}]:")
    test_data = create_test_data(data, 2011, 2016, p)
    start_time = timeit.default_timer()
    test_dict = test(test_loader)
...
max_test_score = max(test_curve)
print("------------------------------------")
print("------------------------------------")
print("best val score: ", max_val_score)
print("best validation epoch   : ", best_eval_idx + 1)
