...
def main():
    ...
    src = torch.from_numpy(dataset.dataset.full_data["sources"])
    dst = torch.from_numpy(dataset.dataset.full_data["destinations"])
    t = torch.from_numpy(dataset.dataset.full_data["timestamps"])
    edge_label = torch.from_numpy(dataset.dataset.full_data["edge_label"])
    msg = torch.from_numpy(dataset.dataset.full_data["edge_feat"])
    ...
    data = TemporalData(src=src,dst=dst,t=t,msg = msg, y=edge_label)
    ipdb.set_trace()
